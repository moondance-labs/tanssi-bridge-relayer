...
type ScanCommitmentsResult struct {
	...
	Proof            common.SimplifiedMMRProof
	BlockHash        types.Hash
	Error            error
}
...
func makeProof(meta *types.Metadata, api *gsrpc.SubstrateAPI, blockNumber uint32, blockHash types.Hash) (bool, common.SimplifiedMMRProof, error) {
	proof1, err := common.GenerateMMRProofForSolochain(api.Client, blockNumber, blockHash)
	if err != nil {
		return false, common.SimplifiedMMRProof{}, fmt.Errorf("mmr_generateProof(%v, %v): %w", blockNumber, blockHash.Hex(), err)
	}

	proof2, err := common.ConvertToSimplifiedMMRProof(
		proof1.BlockHash,
	...
	if err != nil {
		return false, common.SimplifiedMMRProof{}, fmt.Errorf("simplified proof conversion for block %v: %w", proof1.BlockHash.Hex(), err)
	}

	proofIsValid, err := verifyProof(meta, api, proof2)
	if err != nil {
		return false, common.SimplifiedMMRProof{}, fmt.Errorf("proof verification: %w", err)
	}
...
func verifyProof(meta *types.Metadata, api *gsrpc.SubstrateAPI, proof common.SimplifiedMMRProof) (bool, error) {
	...
	actualRoot := common.CalculateMerkleRoot(&proof, leafHash)
	if err != nil {
		return false, err
	}
...
func fetchCommitmentAndProof(meta *types.Metadata, api *gsrpc.SubstrateAPI, beefyBlockHash types.Hash) (*types.SignedCommitment, *common.SimplifiedMMRProof, error) {
