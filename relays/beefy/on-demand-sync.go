...
func NewOnDemandRelay(config *Config, ethereumKeypair *secp256k1.Keypair) (*OnDemandRelay, error) {
	...
	assetHubChannelID, err := types.HexDecodeString(config.OnDemandSync.ChannelID)
	if err != nil {
...
func (relay *OnDemandRelay) queryNonces(ctx context.Context) (uint64, uint64, error) {
	paraNonce, err := relay.fetchRelayChainNonce(ctx)
	if err != nil {
...
func (relay *OnDemandRelay) fetchRelayChainNonce(_ context.Context) (uint64, error) {
	paraNonceKey, err := types.CreateStorageKey(
		relay.relaychainConn.Metadata(), "EthereumOutboundQueue", "Nonce",
	...
	ok, err := relay.relaychainConn.API().RPC.State.GetStorageLatest(paraNonceKey, &paraOutboundNonce)
	if err != nil {
		return 0, fmt.Errorf(
