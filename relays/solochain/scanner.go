...
func (s *Scanner) Scan(ctx context.Context, beefyBlockNumber uint64) ([]*Task, error) {
	...
	tasks, err := s.findTasks(ctx, beefyBlockNumber-1, beefyBlockMinusOneHash)
	if err != nil {
		return nil, err
	}
...
func (s *Scanner) findTasks(
	...
	nonceKey, err := types.CreateStorageKey(s.relayConn.Metadata(), "EthereumOutboundQueue", "Nonce", s.config.ChannelID[:], nil)
	if err != nil {
		return nil, fmt.Errorf("create storage key for parachain outbound queue nonce with channelID '%v': %w", s.config.ChannelID, err)
	}
	var nonce types.U64
	ok, err := s.relayConn.API().RPC.State.GetStorage(nonceKey, &nonce, relayBlockHash)
	if err != nil {
		return nil, fmt.Errorf("fetch nonce from parachain outbound queue with key '%v' and hash '%v': %w", nonceKey, relayBlockHash, err)
	}
	if !ok {
		log.WithFields(log.Fields{
			"nonceKey":  nonceKey,
			"blockHash": relayBlockHash,
...
func (s *Scanner) findTasksImpl(
	...
	log.WithFields(log.Fields{
		"channelID":         channelID,
		"nonce":             startingNonce,
		"latestBlockNumber": relayBlockNumber,
	}).Debug("Searching backwards from latest block on parachain to find block with nonce")

	messagesKey, err := types.CreateStorageKey(s.relayConn.Metadata(), "EthereumOutboundQueue", "Messages", nil, nil)
	if err != nil {
	...
	for currentBlockNumber := relayBlockNumber; currentBlockNumber > 0; currentBlockNumber-- {
		...
		blockHash, err := s.relayConn.API().RPC.Chain.GetBlockHash(currentBlockNumber)
		if err != nil {
			return nil, fmt.Errorf("fetch block hash for block %v: %w", currentBlockNumber, err)
		}

		header, err := s.relayConn.API().RPC.Chain.GetHeader(blockHash)
		if err != nil {
		...
		raw, err := s.relayConn.API().RPC.State.GetStorageRaw(messagesKey, blockHash)
		if err != nil {
		...
		if len(result.proofs) > 0 {
			task := Task{
				...
				ProofInput: &ProofInput{
					RelayBlockNumber: currentBlockNumber,
					RelayBlockHash:   blockHash,
				},
			}
